...
nwss <- full_scrape$NWSS_Wastewater_Metric %>% 
  ...
  mutate(date = as.Date(date), 
         ...
         mmwr_date = paste0(mmwr$MMWRyear, '-', 
                     if_else(nchar(mmwr$MMWRweek) == 1, paste0("0", 
                        mmwr$MMWRweek),  as.character(mmwr$MMWRweek)))) %>%
  select(state = wwtp_jurisdiction, key_plot_id, mmwr_date, pcr_conc_lin, population_served, 
         detect_prop_15d) %>% 
  # Frist by site
  ...
  summarise(total_pop = sum(pop_served, na.rm = T), 
            ...
            avg_detect = mean(avg_detect, na.rm = T), .groups = "keep") %>%
  mutate(detect_per_pop = avg_detect/total_pop) %>%
  ungroup() %>%
...
ca_test <- joined_df %>% filter(state == 'CA')
  
model <- ccf(ca_test$percent_pos, ca_test$total_admissions_all_covid_confirmed)
test_positivity_ccf = tibble("acf" = model$acf, "lag" = model$lag) %>%
  arrange(desc(acf)) %>%
  slice_head(n = 1) %>% 
  pull(acf) %>%
  as.numeric()

model <- ccf(ca_test$count_pos, ca_test$total_admissions_all_covid_confirmed)
count_test_positivity_ccf = tibble("acf" = model$acf, "lag" = model$lag) %>%
  arrange(desc(acf)) %>%
  slice_head(n = 1) %>% 
  pull(acf) %>%
  as.numeric()

model <- ccf(ca_test$percent_visits_covid, ca_test$total_admissions_all_covid_confirmed, na.action = na.pass)
nssp_ccf = tibble("acf" = model$acf, "lag" = model$lag) %>%
  arrange(desc(acf)) %>%
  slice_head(n = 1) %>% 
  pull(acf) %>%
  as.numeric()
...
model <- ccf(ca_test$number_tested, ca_test$total_admissions_all_covid_confirmed, na.action = na.pass)
nssp_ccf = tibble("acf" = model$acf, "lag" = model$lag) %>%
  arrange(desc(acf)) %>%
  slice_head(n = 1) %>% 
  pull(acf) %>%
